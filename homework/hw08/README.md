...
The executable will be created in `homework/hw08/build/` and will be named `hw08`.
...
- **Run:** `Tasks: Run Task → run: hw08`
- **Debug:** pick **Debug hw08** and press **F5**

### Option B: CMake CLI (inside `homework/hw08`)
...
```
cd homework/hw08 && cmake -S . -B build && cmake --build build && ./build/hw08 cd ../..
```
...
```
cd homework/hw08
g++ -std=c++17 -Wall -Wextra -Wpedantic -I include src/*.cpp -o hw08
./hw08
```
...
- The target name = folder name (`hw08` → `hw08` binary).
